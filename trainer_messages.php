<?php
    ...
    include('database_connect.php');
        ...
        if($result = $con->query($query)) {
             ...
             if($rowcount == 0) { $output = "<div id='msgbox'><p>No messages yet.</p></div>"; }
                ...
                <p class='sidenav_btn_txt'>My Members<p>
  ...
  return $output;
    ...
    $type = "";
            ...
            if($type_result = $con->query($query)) {
        ...
        $query = "SELECT * from main_members_table where TrainerId = $id";  
        ...
        if($result = $con -> query($query)) {
           ...
           $counter++;
            ...
            $output = $output . "</ul>";
        } else { $output = $output ." ". mysqli_error($con);}
    }
        return $output;
    }
?>
